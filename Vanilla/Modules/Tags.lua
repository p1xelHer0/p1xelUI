-- MAINLINE UnitEffectiveLevel instead of UnitLevel
local _, p1xelUI = ...
...
function m:ElvUITags()
  ...
  local difficultyColor = function(unit, isBattlePet)
    ...
    if isBattlePet then
      ...
      color = GetCreatureDifficultyColor(UnitLevel(unit))
    end
  ...
  ElvUF.Tags.Methods[tagPhase] = function(unit)
    if
      UnitIsPlayer(unit) and UnitIsConnected(unit) and (not UnitInPhase(unit))
      or nil
    then
